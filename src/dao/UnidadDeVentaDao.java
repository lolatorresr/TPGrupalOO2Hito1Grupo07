...
public class UnidadDeVentaDao {
	...
	public UnidadDeVenta traerUnidadVenta(String nombreComercial) {
		...
		try {
			iniciaOperacion();
			p = (UnidadDeVenta) session.createQuery("from UnidadDeVenta u where u.nombreComercial = :nombreComercial")
					.setParameter("nombreComercial", nombreComercial).uniqueResult();
		} finally {
			session.close();
		}
		return p;
...
//----METODOS FOODTRUCK----
	...
	public List<FoodTruck> traerFoodTrucksConElectricidad() {
		List<FoodTruck> lista = null;
		try {
			iniciaOperacion();
			lista = session.createQuery("from FoodTruck f where f.conexionElectrica = true", FoodTruck.class)
					.getResultList();
		} finally {
			if (session != null && session.isOpen())
				session.close();
...
//----METODOS PUESTO DESARMABLE----
	...
	public UnidadDeVenta traerUnidadDeVentaYPersonal(int idUnidadDeVenta) {
		...
		try {
			...
			objeto = (UnidadDeVenta) session.createQuery(hql).setParameter("id", idUnidadDeVenta).uniqueResult();
		} finally {
			session.close();
		}
		return objeto;
	...
	public UnidadDeVenta traerUnidadVentaCompleta(int idUnidadDeVenta) throws HibernateException {
		...
		Session session = HibernateUtil.getSessionFactory().openSession();
		try {
			String hql = "select distinct u from UnidadDeVenta u " + "left join fetch u.pedidos p "
					+ "left join fetch p.itemPlato ip " + "left join fetch ip.plato " + "where u.idUnidadDeVenta = :id";
			uv = (UnidadDeVenta) session.createQuery(hql).setParameter("id", idUnidadDeVenta).uniqueResult();
		} catch (HibernateException he) {
			throw he;
		} finally {
			session.close();
	...
	public List<UnidadDeVenta> traerUnidadVentaMayorSuperficie(double superficie) throws HibernateException {
		...
		try {
			...
			lista = session.createQuery(hql, UnidadDeVenta.class).setParameter("superficie", superficie)
					.getResultList();
		} finally {
			session.close();
		}
